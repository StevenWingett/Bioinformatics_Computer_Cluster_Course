...
2. Using FlieZilla, create a folder in your cluster home directory named `LMB_Cluster_Course_Exercises`.

Note: your home directory will be your username (e.g. `/lmb/home/jsmith`).

3. Copy the data files given to you at the start of this course into the directory on the cluster you just created. (You can extract the individual files from the TAR archive first with WinZip, or similar, if you prefer).
...
2.  Create a folder in here named `Exercise2`.

3.  Rename this directory to `Exercise_2`.
...
3.  We’ve not introduced the touch command so far, so let’s try it now.  Enter in the command line: `touch file1.txt`
...
3.    Now try the command again, but this type append the output to `date.txt`.
...
2.    Repeat the previous question, but this time edit the file `/usr/bin/who`.  Were you able to do this?  If not, why not?  Maybe checking the file permissions will clarify the situation?
...
#### d
1.    Write the contents of the `$USER` variable to the screen.  Look familiar?
